...
class SettingsDataManager(
    ...
    val cameraAgcFlow: Flow<Boolean> =
        appContext.dataStore.data.map { prefs ->
            prefs[CAMERA_AGC_KEY] == true
        }

    val cameraEmissivityFlow: Flow<String> =
        appContext.dataStore.data.map { prefs ->
            prefs[CAMERA_EMISSIVITY_KEY] ?: "90"
        }

    val cameraGainModeFlow: Flow<Int> =
        appContext.dataStore.data.map { prefs ->
    ...
    suspend fun getExportPicture(): Boolean = exportPictureFlow.first()

    suspend fun getExportMetadata(): Boolean = exportMetadataFlow.first()

    suspend fun getExportResolution(): String = exportResolutionFlow.first()

    suspend fun getManualRange(): Boolean = manualRangeFlow.first()

    suspend fun getMinValue(): String = minValueFlow.first()

    suspend fun getMaxValue(): String = maxValueFlow.first()

    suspend fun getSelectedPalette(): String = selectedPaletteFlow.first()
    ...
    suspend fun isUnitsCelsius(): Boolean = temperatureUnitFlow.first() == "Celsius"

    suspend fun isUnitsFahrenheit(): Boolean = temperatureUnitFlow.first() == "Fahrenheit"

    suspend fun isManualRange(): Boolean = manualRangeFlow.first()
